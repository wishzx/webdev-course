...
var commentSchema = new mongoose.Schema({
    ...
    user_id: { type: mongoose.Schema.Types.ObjectId, index: true },    // 	The ID of the user who created the comment.
});
...
var photoSchema = new mongoose.Schema({
    ...
    user_id: { type: mongoose.Schema.Types.ObjectId, index: true }, // The ID of the user who created the photo.
    comments: [commentSchema] // Array of comment objects representing the comments made on this photo.
});

//return the comments in a photo of a particular user
photoSchema.methods.userComments = function userComments(user_id) {
    const filtered_array = this.comments.filter(comment => comment.user_id == user_id);
